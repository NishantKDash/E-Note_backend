...
router.post('/createUser' ,[
  ...
  try {
    ...
    if(user){
        return res.status(400).json({error : " Sorry an user with this email id exists already"});
    }
   //creating a salt
 ...
 //destructuring the email and password what we got
  ...
  try{
      ...
      //just like select command in sql
          ...
          if(!user)
          {
             return res.status(400).json({error : "Please try to login with correct credentials"}); // not correct email as we dont want to give out the information that email is wrong
          }
